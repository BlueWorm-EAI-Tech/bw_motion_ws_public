...
def generate_launch_description():
    ...
    # 4. Robot State Publisher - 通过parameters传递robot_description
    node_rsp = Node(
    ...
    # 5. View launch：只负责控制相关节点（router/playback/panel），不启动 RViz
    ...
    # 7. 功能节点：Bridge（phase1 启动时包含实机桥接）
    node_bridge = Node(
        package="bw_sim2real",
    ...
    # 8. 功能节点：IK（注意：该节点较重，后续可改为参数化可选启动）
    node_casadi_ik = Node(
        package="bw_sim2real",
